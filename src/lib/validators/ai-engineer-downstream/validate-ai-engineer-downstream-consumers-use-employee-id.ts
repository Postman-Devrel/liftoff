  ...
  async (apiKey, context) => {
    ...
    if (requests.length === 0) {
      return {
        success: false,
        message:
          'The "Enterprise Resource Planning" collection has no requests to inspect.',
        pointsAwarded: 0,
      };
    }
    ...
    if (employeeIdReferences === 0) {
      return {
        success: false,
        message:
          'No requests in the "Enterprise Resource Planning" collection reference `employee-id`. Pull the merged downstream updates and re-validate.',
        pointsAwarded: 0,
      };
    }
    ...
    return {
      success: true,
      message: `Verified — ${employeeIdReferences} request(s) in the "Enterprise Resource Planning" collection reference \`employee-id\`.${legacyNote}`,
      pointsAwarded: 10,
      context,
    };
  };
