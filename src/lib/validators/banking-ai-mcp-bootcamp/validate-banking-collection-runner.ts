...
export const validateBankingCollectionRunner: ValidatorFn = async (apiKey, context) => {
      ...
      if (item.request) {
        ...
        if (item.event) {
          ...
          if (testEvent?.script?.exec && testEvent.script.exec.length > 0) {
            ...
            if (/pm\.test/i.test(scriptContent)) {
              requestsWithTests++;
            }
          }
        }
  ...
  if (requestsWithTests === 0) {
    return {
      success: false,
      message: "No test scripts found in the collection. Complete the previous step first.",
      pointsAwarded: 0,
    };
  }
  ...
  if (totalRequests > 0 && requestsWithTests < totalRequests) {
    return {
      success: false,
      message: `Only ${requestsWithTests} of ${totalRequests} requests have tests. All requests need tests before running the Collection Runner.`,
      pointsAwarded: 0,
    };
  }
  ...
  return {
    success: true,
    message: `All ${requestsWithTests} requests have test scripts. Run the Collection Runner and verify all tests pass to complete this step.`,
    pointsAwarded: 10,
    context,
  };
};
