...
export const validateBankingFromAccount: ValidatorFn = async (apiKey, context) => {
  ...
  const effectiveValue = resolveEnvVar(
    values,
    "fromAccount",
    'Post-response script found, but "fromAccount" variable is not set. Send the fromAccount request to populate it, then Share/Persist the environment.'
  );
  if (typeof effectiveValue !== "string") return effectiveValue;

  return {
    success: true,
    message: `Post-response script and variable "fromAccount" are both set!`,
    pointsAwarded: 10,
