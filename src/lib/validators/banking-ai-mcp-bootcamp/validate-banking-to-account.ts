...
export const validateBankingToAccount: ValidatorFn = async (apiKey, context) => {
  ...
  if (!toAccountReq) {
    return { success: false, message: 'Could not find a "toAccount" request in the collection.', pointsAwarded: 0 };
  }

  if (!hasPostResponseScript(toAccountReq, /toAccount|accountId/i)) {
    return { success: false, message: 'The toAccount request does not have a post-response script that saves the accountId. Use Agent Mode to add one.', pointsAwarded: 0 };
  }
