...
export const validateBankingTransactionId: ValidatorFn = async (apiKey, context) => {
  ...
  if (!transactionReq) {
    return {
      success: false,
      message: 'Could not find a "Create new transaction" request in the collection.',
      pointsAwarded: 0,
    };
  }
  ...
  if (!hasPostResponseScript(transactionReq, /transactionId/i)) {
    return {
      success: false,
      message: 'The Create new transaction request does not have a post-response script that saves the transactionId. Use Agent Mode to add one.',
      pointsAwarded: 0,
    };
  }
