...
export const validateBankingTransactionId: ValidatorFn = async (apiKey, context) => {
  ...
  const transactionReq = findRequest(items, /new\s+transaction|create.*transaction/i);
  if (!transactionReq) {
    return { success: false, message: 'Could not find a "Create new transaction" request in the collection.', pointsAwarded: 0 };
  }

  if (!hasPostResponseScript(transactionReq, /transactionId/i)) {
    return { success: false, message: 'The Create new transaction request does not have a post-response script that saves the transactionId. Use Agent Mode to add one.', pointsAwarded: 0 };
  }
  ...
  const effectiveValue = resolveEnvVar(
    values,
    "transactionId",
    'Post-response script found, but "transactionId" variable is not set. Send the Create new transaction request to populate it, then Share/Persist the environment.'
  );
  if (typeof effectiveValue !== "string") return effectiveValue;

  return {
    success: true,
    message: `Post-response script and variable "transactionId" are both set!`,
    pointsAwarded: 10,
